          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item,index) in spuSaleAttrList" :key="item.id">
                ...
                <dd @click="changeActive(item.spuSaleAttrValueList,item2)" changepirce="0" :class="{active:item2.isChecked==1}" v-for="(item2,index) in item.spuSaleAttrValueList" :key="item2.id">{{item2.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
<script>
  ...
  export default {
    ...
    methods:{
      async addShopCart(){
        try {
          await this.$store.dispatch('getCartSuccess',{skuId:this.$route.params.skuid,skuNum:this.skuNum})
          this.$router.push({name:'addcartsuccess',query:{skuNum:this.skuNum}})
          //我们通过会话存储来传递数据skuInfo
      ...
      changeActive(saleAttr,activeInfo){
        saleAttr.forEach(element => {
          element.isChecked=0
        });
        activeInfo.isChecked=1
      },
      changeNum(val){
        let value=val*1
